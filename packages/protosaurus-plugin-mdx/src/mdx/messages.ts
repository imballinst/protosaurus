  ...
  if (fields.length === 0) {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      if (
        ...
        if (!field.ismap && innerObjectsRecord[field.longType].messageBlock) {
...
function getFieldType({
  ...
  if (field.ismap && mapType) {
    ...
    const keyType =
      ...
      objectsRecord?.[keyField.longType]?.rawMessage?.name ||
      objectsRecord?.[keyField.longType]?.rawEnum?.name ||
      keyField?.type;
    const valueType =
      innerObjectsRecord?.[valueField.longType]?.rawMessage?.name ||
      innerObjectsRecord?.[valueField.longType]?.rawEnum?.name ||
      objectsRecord?.[valueField.longType]?.rawMessage?.name ||
      objectsRecord?.[valueField.longType]?.rawEnum?.name ||
      valueField?.type;
